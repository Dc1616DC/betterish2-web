...
export function initializeFirebaseClient() {
  ...
  try {
    ...
    // Initialize messaging for push notifications (sync initialization)
    let messaging = null;
    try {
      if (typeof window !== 'undefined') {
      ...
      // Messaging not supported or failed to initialize
...
function getFirebaseInstances() {
  ...
  return firebaseInstances || { app: null, auth: null, db: null, messaging: null };
}
...
export const messaging = typeof window !== 'undefined' ? getFirebaseInstances().messaging : null;
